       ...
       rt3api.getAllAvailableRooms().then(function(response) {
        $rootScope.$applyAsync(function() {
         //console.log(response);
            ...
            var roomCategories = response.rooms.map(function(obj) { return obj.category.trim(); });
            roomCategories = roomCategories.filter(function(v,i) { return roomCategories.indexOf(v) == i; });
            roomCatInReztripOrder = ['Courtyard', 'Partial Ocean View', 'Poolside', 'Pool Terrace', 'Ocean Front'];
            roomCategories.sort(function(a, b){
              return roomCatInReztripOrder.indexOf(a) - roomCatInReztripOrder.indexOf(b);
            });
